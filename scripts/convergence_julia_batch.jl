...
# Batch CPU convergence verification for Julia MoE Transformer.
#
...
function run_trial(seed::Int, input::Tensor, targets::Tensor, n_steps::Int)
    ...
    for i in 1:n_steps
        losses[i] = train_step!(trainer, input, targets)
    end
    losses
end
...
function main()
    ...
    println(stderr, "[julia/cpu] warmup (seed=0)...")
    run_trial(0, input, targets, n_steps)
    println(stderr, "[julia/cpu] warmup complete")

    # Measurement runs
    all_trials = Vector{Vector{Float32}}(undef, n_trials)
    for trial in 1:n_trials
        println(stderr, "[julia/cpu] trial $trial/$n_trials...")
        all_trials[trial] = run_trial(trial, input, targets, n_steps)
    end

    # Output JSON
    print("{\"language\":\"julia\",\"n_trials\":$n_trials,\"trials\":[")
