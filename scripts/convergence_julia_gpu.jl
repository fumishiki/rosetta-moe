...
# GPU Loss convergence verification for Julia MoE Transformer.
...
function main()
    ...
    losses = Float32[]
    for _ in 1:n_steps
        push!(losses, gpu_train_step!(trainer, input, targets))
    end

    # Output JSON
    loss_strs = join([@sprintf("%.6f", l) for l in losses], ",")
    println("{\"language\":\"julia_gpu\",\"steps\":$n_steps,\"losses\":[$loss_strs]}")
end
