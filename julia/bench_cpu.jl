...
# bench_cpu.jl — CPU-only benchmark harness for MoE Transformer
#
# Runs all CPU scenarios (memory, compiler, type_system, parallel, scale).
# Includes from src_cpu/ only. NO Metal.jl dependency.
#
# Usage: julia --project=. bench_cpu.jl

include("src_cpu/MoETransformerCPU.jl")
using .MoETransformerCPU
import .MoETransformerCPU as MOE
...
function main()
    println(stderr, "Julia MoE Transformer CPU Benchmark")
    println(stderr, "============================================================")
    ...
    push!(scenarios, run_scenario(
        ...
        setup_fn=() -> begin
            ...
            m = tiny_model(); cfg = default_train_config()
            trainer = Trainer(m, cfg)
            input = make_forward_input(2, 8); targets = make_targets(2, 8)
    ...
    for bs in [1, 2, 4, 8]
        push!(scenarios, run_scenario(
            ...
            setup_fn=() -> begin
                Random.seed!(SEED); seed_rng!(SEED)
                m = tiny_model(); input = make_forward_input(bs, 32)
                (model=m, input=input)
            end,
    ...
    for seq in [8, 16, 32, 64]
        push!(scenarios, run_scenario(
            ...
            setup_fn=() -> begin
                Random.seed!(SEED); seed_rng!(SEED)
                m = tiny_model(); input = make_forward_input(2, seq)
                (model=m, input=input)
            end,
            run_fn=ctx -> forward(ctx.model, ctx.input)
        ))
    ...
    push!(scenarios, run_scenario(
        ...
        setup_fn=() -> begin
            Random.seed!(SEED)
            a = randn(Float32, 64, 64); b = randn(Float32, 64, 64)
            bt = collect(transpose(b)); out = Matrix{Float32}(undef, 64, 64)
            (a=a, bt=bt, out=out)
        end,
        run_fn=ctx -> begin
            mul!(ctx.out, ctx.a, ctx.bt); Tensor(ctx.out, F32)
        end,
        known_flops=524288
    ))
    ...
    push!(scenarios, run_scenario(
        ...
        setup_fn=() -> begin
            Random.seed!(SEED); v = randn(Float32, 1000); buf = similar(v)
            (v=v, buf=buf)
        end,
        run_fn=ctx -> begin
            copyto!(ctx.buf, ctx.v); softmax_in_place!(ctx.buf)
    ...
    push!(scenarios, run_scenario(
        ...
        setup_fn=() -> begin
            Random.seed!(SEED); t = randn_tensor(2, 32, 64); norm = RMSNorm(64)
            (t=t, norm=norm)
        end,
    ...
    push!(scenarios, run_scenario(
        ...
        setup_fn=() -> begin
            Random.seed!(SEED); seed_rng!(SEED)
            m = tiny_model(); input = make_forward_input(2, 32)
            (model=m, input=input)
        end,
        run_fn=ctx -> forward(ctx.model, ctx.input)
    ))
    ...
    push!(scenarios, run_scenario(
        ...
        n_warmup=N_WARMUP,
        setup_fn=() -> begin
            Random.seed!(SEED); input = make_forward_input(1, 8)
            (input=input,)
        end,
        run_fn=ctx -> begin
            seed_rng!(SEED); m = tiny_model(); forward(m, ctx.input)
        end
    ))
    ...
    for T in [1, 2, 4]
        ...
        push!(scenarios, run_scenario(
            ...
            params=Dict{String,Any}("thread_count" => nT, "available_threads" => available_threads,
                                    "batch" => 2, "seq_len" => 32, "hidden_dim" => 64),
            setup_fn=() -> begin
                Random.seed!(SEED); seed_rng!(SEED)
                models = [tiny_model() for _ in 1:nT]
                inputs = [make_forward_input(2, 32) for _ in 1:nT]
    ...
    for T in [1, 2, 4]
        ...
        push!(scenarios, run_scenario(
            ...
            params=Dict{String,Any}("thread_count" => nT, "available_threads" => available_threads,
                                    "batch" => 2, "seq_len" => 8, "hidden_dim" => 64, "workload" => "train_step"),
            setup_fn=() -> begin
                Random.seed!(SEED); seed_rng!(SEED)
                models = [tiny_model() for _ in 1:nT]
                cfgs = [default_train_config() for _ in 1:nT]
                trainers = [Trainer(models[i], cfgs[i]) for i in 1:nT]
                inputs = [make_forward_input(2, 8) for _ in 1:nT]
                targets_list = [make_targets(2, 8) for _ in 1:nT]
    ...
    push!(scenarios, run_scenario(
        ...
        setup_fn=() -> begin
            Random.seed!(SEED); seed_rng!(SEED)
            m = small_model(); input = make_forward_input(2, 32)
            (model=m, input=input)
        end,
        run_fn=ctx -> forward(ctx.model, ctx.input)
    ))
    ...
    push!(scenarios, run_scenario(
        ...
        setup_fn=() -> begin
            Random.seed!(SEED); seed_rng!(SEED)
            m = small_model(); cfg = default_train_config()
            trainer = Trainer(m, cfg)
            input = make_forward_input(2, 8); targets = make_targets(2, 8)
            (trainer=trainer, input=input, targets=targets)
        end,
    ...
    push!(scenarios, run_scenario(
        id="scale_forward_512", axis="scale",
        params=Dict{String,Any}("batch" => 2, "seq_len" => 32, "hidden_dim" => 512),
        setup_fn=() -> begin
            Random.seed!(SEED); seed_rng!(SEED)
            m = medium_model(); input = make_forward_input(2, 32)
            (model=m, input=input)
        end,
        run_fn=ctx -> forward(ctx.model, ctx.input)
    ))
    ...
    push!(scenarios, run_scenario(
        id="scale_train_512", axis="scale",
        params=Dict{String,Any}("batch" => 2, "seq_len" => 8, "hidden_dim" => 512),
        setup_fn=() -> begin
            Random.seed!(SEED); seed_rng!(SEED)
            m = medium_model(); cfg = default_train_config()
            trainer = Trainer(m, cfg)
            input = make_forward_input(2, 8); targets = make_targets(2, 8)
            (trainer=trainer, input=input, targets=targets)
        end,
...
"""
    ...
    print(json)
    flush(stdout)
    println(stderr, "CPU Benchmark complete. $(length(scenarios)) scenarios.")
    flush(stderr)
end
