...
# MoETransformerCPU.jl — CPU-only module definition
#
# Entry point for the CPU-only Mixture-of-Experts Transformer implementation.
# All sub-files are included in dependency order; the module re-exports
# the full public API so callers only need `using .MoETransformerCPU`.
# NO Metal.jl or MtlArray references in this module.

module MoETransformerCPU
