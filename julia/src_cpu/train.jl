...
struct TrainConfig
    ...
    z_loss_weight::Float32    # Router z-loss coefficient (ST-MoE)
    routing_mode::RoutingMode # routing strategy: TopK/BiasFree/ReLU
    bias_gamma::Float32       # BiasFree bias update rate
    relu_lambda_l1::Float32   # ReLU L1 regularization strength
    relu_target_k::Int        # ReLU target active experts
end

default_train_config() = TrainConfig(1f-4, 0.9f0, 0.95f0, 1f-8, 0.1f0, 0.5f0, 1000, 100000, 0.01f0, 0.05f0, TopKMode, 0.001f0, 0.01f0, 2)
...
# Train step from pre-computed logits: loss + backward + optimizer.
# Used by both CPU train_step! and GPU gpu_train_step! (which computes logits on GPU).
function train_step_from_logits!(t::Trainer, logits::Tensor, targets::Tensor)::Float32
    t.step += 1
    ...
    # Cross-entropy loss only (auxiliary losses applied after backward)
    total_loss = loss

    clip_grad_by_global_norm!(grad_output, t.config.grad_clip)
    backward(t.model, grad_output)

    # Apply auxiliary losses with gradient backprop (after backward, before optimizer)
    if t.config.routing_mode == TopKMode
        # TopK: both aux_loss and z_loss
        aux_loss = apply_aux_loss!(t.model, t.config.aux_alpha)
        z_loss = apply_z_loss!(t.model, t.config.z_loss_weight)
        total_loss += aux_loss + z_loss
    elseif t.config.routing_mode == BiasFreeMode
        # BiasFree: z_loss only
        z_loss = apply_z_loss!(t.model, t.config.z_loss_weight)
        total_loss += z_loss
    elseif t.config.routing_mode == ReLUMode
        # ReLU mode: L1 loss instead
        relu_l1 = apply_relu_l1_loss!(t.model)
        total_loss += relu_l1
    end
    lr = get_lr(t)
    step = t.step
    ...
    # Post-step routing-mode updates
    if t.config.routing_mode == BiasFreeMode
        update_routing_biases!(t.model, t.config.bias_gamma)
    end
...
function train_step!(t::Trainer, input::Tensor, targets::Tensor)::Float32
    ...
    train_step_from_logits!(t, logits, targets)
end
