...
class MoETransformer:
    ...
    def apply_z_loss(self, z_weight: float = 0.01) -> float:
        """Compute and backprop z-loss from all routers.

        Z-loss: L_z = z_weight * (1/B) * sum_i(logsumexp(logits_i)^2)

        Returns total z-loss summed across all transformer blocks.
        """
        total_z_loss = 0.0
        for block in self.blocks:
            total_z_loss += block.moe.router.compute_z_loss_with_grad(z_weight)
        return total_z_loss

    def apply_aux_loss(self, alpha: float = 0.01) -> float:
        """Compute and backprop aux loss from all routers.

        Aux loss: L_aux = alpha * N * sum_e(f_e * P_e)

        Returns total aux loss summed across all transformer blocks.
        """
        total_aux_loss = 0.0
        for block in self.blocks:
            total_aux_loss += block.moe.router.compute_aux_loss_with_grad(alpha)
        return total_aux_loss

    def set_routing_mode(self, mode) -> None:
        """Set routing mode for all MoE layers.

        Args:
            mode: RoutingMode enum value
        """
        for block in self.blocks:
            block.moe.router.routing_mode = mode

    def update_routing_biases(self, gamma: float) -> None:
        """Update expert biases in all routers (BiasFree mode).

        Args:
            gamma: Learning rate for bias update
        """
        for block in self.blocks:
            block.moe.router.update_expert_bias(gamma)

    def apply_relu_l1_loss(self) -> float:
        """Compute ReLU L1 loss with gradient backprop.

        Returns:
            Total L1 loss summed across all routers
        """
        total_l1 = 0.0
        for block in self.blocks:
            total_l1 += block.moe.router.compute_relu_l1_loss_with_grad()
        return total_l1

    def avg_active_experts(self) -> float:
        """Return average active experts across all routers (ReLU mode).

        Returns:
            Average number of active experts per token
        """
        if not self.blocks:
            return 0.0
        avg = sum(block.moe.router.last_avg_active for block in self.blocks)
        return avg / len(self.blocks)
