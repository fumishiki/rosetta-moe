...
# Knuth's MMIX constants (same as generate.py sampling, Rust/Go/Julia init).
_LCG_MULT = 6_364_136_223_846_793_005
_LCG_MASK = (1 << 64) - 1  # 0xFFFFFFFFFFFFFFFF
_lcg_state: int = 42
...
def _lcg_uniform() -> float:
    """Return a uniform random float in [1e-10, 1.0] using Knuth LCG."""
    global _lcg_state
    _lcg_state = (_lcg_state * _LCG_MULT + 1) & _LCG_MASK
    u = _lcg_state / 18_446_744_073_709_551_615  # u64::MAX
    return max(u, 1e-10)


def _lcg_normal() -> float:
    """Return a standard normal sample using Box-Muller + LCG."""
    global _lcg_cache
...
class Tensor:
    ...
    def randn(cls, shape: Sequence[int], dtype: DType = DType.F32) -> Tensor:
        """Create tensor with random normal values using portable LCG."""
        n = 1
        for s in shape:
            n *= s
        ...
        for i in range(n):
            data[i] = _lcg_normal()
        return cls(data.reshape(shape).astype(dtype.to_numpy()), dtype)

    @classmethod
    def randn_std(
        cls, shape: Sequence[int], dtype: DType = DType.F32, std: float = 1.0
    ) -> Tensor:
        """Create tensor with random normal values and specific std, using portable LCG."""
        n = 1
        for s in shape:
            n *= s
        data = np.empty(n, dtype=np.float32)
