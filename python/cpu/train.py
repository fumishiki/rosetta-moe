...
class RoutingMode(Enum):
    """Routing strategy for MoE layer."""
    TOPK = "topk"          # current (softmax + greedy top-k + aux_loss + z_loss)
    BIAS_FREE = "biasfree"  # DeepSeek-V3 (softmax + bias-augmented top-k)
    RELU = "relu"           # ReMoE (ReLU gate + L1 regularization)
...
class TrainConfig:
    ...
    z_loss_weight: float = 0.05  # Phase 0: increased from 0.01 to 0.05
    routing_mode: RoutingMode = RoutingMode.TOPK
    bias_gamma: float = 0.001
    relu_lambda_l1: float = 0.01
    relu_target_k: int = 2
...
class Trainer:
    ...
    def train_step_from_logits(self, logits: Tensor, targets: Tensor) -> float:
        """Train step with pre-computed logits (GPU hybrid: forward on GPU, backward on CPU).

        Args:
            logits: Pre-computed logits [batch, seq_len, vocab]
            targets: Target token IDs [batch, seq_len]
        ...
        # Routing mode branching
        if self.config.routing_mode == RoutingMode.TOPK:
            # TopK: aux_loss + z_loss (both with gradient backprop)
            aux_loss = self.model.apply_aux_loss(self.config.aux_loss_alpha)
            z_loss = self.model.apply_z_loss(self.config.z_loss_weight)
            total_loss += aux_loss + z_loss

        elif self.config.routing_mode == RoutingMode.BIAS_FREE:
            # BiasFree: z_loss only
            z_loss = self.model.apply_z_loss(self.config.z_loss_weight)
            total_loss += z_loss

        elif self.config.routing_mode == RoutingMode.RELU:
            # ReMoE: ReLU L1 regularization
            relu_l1 = self.model.apply_relu_l1_loss()
            total_loss += relu_l1
        ...
        # Post-step adjustments
        if self.config.routing_mode == RoutingMode.BIAS_FREE:
            # Update expert biases
            self.model.update_routing_biases(self.config.bias_gamma)

        elif self.config.routing_mode == RoutingMode.RELU:
            # Adaptive lambda adjustment
            avg_active = self.model.avg_active_experts()
            target_k = self.config.relu_target_k
            if avg_active > target_k + 0.5:
                # Too many active experts, increase regularization
                for block in self.model.blocks:
                    block.moe.router.relu_lambda_l1 *= 1.01
            elif avg_active < target_k - 0.5:
                # Too few active experts, decrease regularization
                for block in self.model.blocks:
                    block.moe.router.relu_lambda_l1 *= 0.99
    ...
    def train_step(self, input_ids: Tensor, targets: Tensor) -> float:
        ...
        Forward pass followed by backward and optimizer update.
        ...
        return self.train_step_from_logits(logits, targets)
