...
STUB_IMPL(cuda_scale, const float* input, float scale, float* output, int64_t n, void* stream)
...
STUB_IMPL(cuda_gemm_beta, const float* A, const float* B, float* C,
          int M, int N, int K, float alpha, float beta, void* stream)
STUB_IMPL(cuda_batched_gemm, const float* A, const float* B, float* C,
...
STUB_IMPL(cuda_flash_attention, const float* Q, const float* K, const float* V, float* output,
          int batch, int n_q_heads, int n_kv_heads, int seq_len, int head_dim, float scale, void* stream)
