...
impl CudaGraph {
    ...
    pub fn begin_capture(&mut self) -> FfiResult<()> {
        if self.state != GraphState::Uninitialized {
            return Err(FfiError(1)); // Already capturing or ready
        }
    ...
    pub fn end_capture(&mut self) -> FfiResult<()> {
        if self.state != GraphState::Capturing {
            return Err(FfiError(2)); // Not capturing
        }
    ...
    pub fn launch(&mut self, stream: *mut c_void) -> FfiResult<()> {
        if self.state != GraphState::Ready {
            return Err(FfiError(3)); // Not ready
        }
...
impl GraphExecutor {
    ...
    pub fn begin(&mut self) -> FfiResult<ExecutionHandle> {
        match self.mode {
            ...
            GraphExecutionMode::GraphOnly => {
                if self.graph.is_ready() {
                    ...
                    Err(FfiError(4)) // Graph required but not available
                }
            }
        }
    }
    ...
    pub fn end(&mut self, handle: ExecutionHandle, stream: *mut c_void) -> FfiResult<()> {
        match handle {
