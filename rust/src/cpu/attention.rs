...
use super::layers::{Layer, Linear, collect_params, collect_params_mut};
use super::tensor::{self, Shape, Tensor};
...
impl MQAttention {
    ...
    /// Read-only access to Q/K/V/O projection weight data (for GPU upload).
    pub fn q_weight_data(&self) -> &[f32] { self.q_proj.weight.data() }
    pub fn k_weight_data(&self) -> &[f32] { self.k_proj.weight.data() }
    pub fn v_weight_data(&self) -> &[f32] { self.v_proj.weight.data() }
    pub fn o_weight_data(&self) -> &[f32] { self.o_proj.weight.data() }
