...
use super::tensor::{DType, Shape, Tensor};
...
impl Embedding {
    pub fn new(vocab_size: usize, hidden_dim: usize) -> Self {
        ...
        Self {
            weight: Tensor::randn(Shape::new(&[vocab_size, hidden_dim]), DType::F32).scale(std),
            last_ids: None,
        }
    }
...
impl Layer for RMSNorm {
    ...
    fn forward(&mut self, input: &Tensor) -> Tensor {
        ...
        for row_idx in 0..outer {
            ...
            rsqrt_in[0] = mean_sq;
            super::simd::fast_rsqrt_slice(&rsqrt_in, self.eps, &mut rsqrt_out);
    ...
    fn backward(&mut self, grad_output: &Tensor) -> Tensor {
        ...
        if let (Some(input_data), Some(inv_rms_vec)) = (&self.last_input, &self.last_inv_rms) {
            ...
            for row_idx in 0..outer {
                ...
                // Compute dot_sum = sum(grad_output * gamma * input)
                let mut dot_sum = 0.0f32;
                for j in 0..hidden {
                    dot_sum += go[start + j] * gamma[j] * input_data[start + j];
                }
...
impl Linear {
    pub fn new(in_features: usize, out_features: usize) -> Self {
        ...
        Self {
            weight: Tensor::randn(Shape::new(&[out_features, in_features]), DType::F32)
                .scale(std),
            last_input: None,
            last_batch: 0,
            dx_buf: None,
...
impl SwiGLU {
    ...
    pub(crate) fn gate_proj(&self) -> &Linear {
        &self.gate_proj
    }

    pub(crate) fn up_proj(&self) -> &Linear {
        &self.up_proj
    }

    pub(crate) fn down_proj(&self) -> &Linear {
        &self.down_proj
    }
}
