...
use super::model::MoETransformer;
use super::tensor::{DType, Tensor};

/// Routing mode for MoE layers.
#[derive(Debug, Clone, Copy, PartialEq)]
pub enum RoutingMode {
    /// TopK: Softmax + greedy top-k + aux_loss + z_loss (current/default)
    TopK,
    /// BiasFree: DeepSeek-V3 style (softmax + bias-augmented top-k + bias update, no aux_loss, z_loss)
    BiasFree,
    /// ReLU: ReMoE style (ReLU gate + L1 regularization + adaptive lambda, no aux_loss/z_loss)
    ReLU,
}
...
impl CrossEntropyLoss {
    ...
    pub fn forward(logits: &Tensor, targets: &Tensor) -> Tensor {
        ...
        for i in 0..n {
            let start = i * vocab;
            super::tensor::softmax_into_slice(&logits_data[start..start + vocab], &mut probs);
...
pub struct TrainConfig {
    ...
    pub bias_gamma: f32,
    pub relu_lambda_l1: f32,
    pub relu_target_k: usize,
}
...
pub struct Trainer {
    ...
    // Last step loss decomposition (diagnostic).
    last_ce_loss: f32,
    last_aux_loss: f32,
    last_z_loss: f32,
    last_grad_norm: f32,
...
impl Trainer {
    ...
    /// Returns (ce_loss, aux_loss, z_loss, grad_norm, clipped) from the last training step.
    pub fn last_loss_components(&self) -> (f32, f32, f32, f32, bool) {
        (self.last_ce_loss, self.last_aux_loss, self.last_z_loss, self.last_grad_norm, self.last_clipped)
    }

    /// Returns per-layer expert token counts from the last forward pass.
    pub fn expert_counts(&self) -> Vec<&[f32]> {
        self.model.expert_counts()
    }
    ...
    pub fn model_mut(&mut self) -> &mut MoETransformer {
        &mut self.model
    }
    ...
    fn cross_entropy_loss_grad(&mut self, logits: &Tensor, targets: &Tensor) -> (f32, Tensor) {
        ...
        for i in 0..n {
            let start = i * vocab;
            super::tensor::softmax_into_slice(&logits_data[start..start + vocab], row_probs);
    ...
    /// Train step with pre-computed logits (GPU hybrid: forward on GPU, backward on CPU).
    pub fn train_step_with_logits(&mut self, logits: Tensor, targets: &Tensor) -> f32 {
        self.current_step += 1;
        ...
        // aux_loss, z_loss, and relu_l1 use router state (last_expert_counts, last_gate_probs, last_logits)
        // which survives backward, so they're computed post-backward with gradient accumulation.
        let (aux_loss, z_loss, relu_l1) = match self.config.routing_mode {
            RoutingMode::TopK => (
                self.model.apply_aux_loss(self.config.aux_loss_weight),
                self.model.apply_z_loss(self.config.z_loss_weight),
                0.0,
            ),
            RoutingMode::BiasFree => (0.0, self.model.apply_z_loss(self.config.z_loss_weight), 0.0),
            RoutingMode::ReLU => (0.0, 0.0, self.model.apply_relu_l1_loss()),
        };

        let total_loss = ce_loss + aux_loss + z_loss + relu_l1;
        self.last_ce_loss = ce_loss;
        self.last_aux_loss = aux_loss;
        self.last_z_loss = z_loss;

        // Update learning rate based on schedule before taking mutable borrow
        let scheduled_lr = self.get_lr();

        // Global gradient norm clipping + optimizer step
        let mut params = self.model.parameters_mut();
        ...
        // Post-step updates based on routing mode
        match self.config.routing_mode {
            RoutingMode::TopK => {}
            RoutingMode::BiasFree => {
                self.model.update_routing_biases(self.config.bias_gamma);
            }
            RoutingMode::ReLU => {
                let avg_active = self.model.avg_active_experts();
                let target_k = self.config.relu_target_k as f32;
                if avg_active > target_k + 0.1 {
                    self.config.relu_lambda_l1 *= 1.01;
                } else if avg_active < target_k - 0.1 {
                    self.config.relu_lambda_l1 *= 0.99;
                }
                self.model.set_relu_lambda(self.config.relu_lambda_l1);
            }
        }
    ...
    pub fn train_step(&mut self, input: &Tensor, targets: &Tensor) -> f32 {
        ...
        self.train_step_with_logits(logits, targets)
    }
