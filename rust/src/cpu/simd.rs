...
//!
//! Accuracy: full f32 precision after two Newton-Raphson refinement steps,
//! exceeding the 24-bit f32 mantissa for optimizer and normalization use cases.
//!
...
/// followed by two Newton-Raphson refinement steps for full f32 precision:
///   x1 = x0 * (3 - v * x0^2) / 2
///   x2 = x1 * (3 - v * x1^2) / 2  (~46-bit accuracy, exceeds f32 precision)
///
...
pub fn fast_rsqrt_slice(v: &[f32], eps: f32, out: &mut [f32]) {
    ...
    for i in 0..chunks {
        ...
        // First Newton-Raphson step: ~23-bit accuracy
        let est_sq = unsafe { vmulq_f32(est, est) };
        let refine1 = unsafe { vrsqrtsq_f32(val_eps, est_sq) };
        let result1 = unsafe { vmulq_f32(est, refine1) };
        // Second Newton-Raphson step: ~46-bit accuracy (exceeds f32 precision)
        let result1_sq = unsafe { vmulq_f32(result1, result1) };
        let refine2 = unsafe { vrsqrtsq_f32(val_eps, result1_sq) };
        let result = unsafe { vmulq_f32(result1, refine2) };
...
pub fn adamw_step_simd(
    ...
    for i in 0..chunks {
        ...
        unsafe {
            ...
            // Approximate rsqrt(v_hat + eps) via NEON estimate + two Newton-Raphson steps
            let v_hat_eps = vaddq_f32(v_hat, eps_vec);
            let est = vrsqrteq_f32(v_hat_eps);
            let est_sq = vmulq_f32(est, est);
            let refine1 = vrsqrtsq_f32(v_hat_eps, est_sq);
            let result1 = vmulq_f32(est, refine1);
            let result1_sq = vmulq_f32(result1, result1);
            let refine2 = vrsqrtsq_f32(v_hat_eps, result1_sq);
            let rsqrt_val = vmulq_f32(result1, refine2);
