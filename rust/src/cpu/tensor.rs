...
thread_local! {
    static RNG_STATE: Cell<u64> = const { Cell::new(42) };
}

/// Seed the global RNG used by `Tensor::randn`.
/// Call before model construction for reproducible initialization.
...
impl Tensor {
    ...
    pub fn randn(shape: Shape, dtype: DType) -> Self {
        RNG_STATE.with(|cell| {
    ...
    pub fn try_matmul(&self, other: &Tensor) -> TensorResult<Tensor> {
            ...
            .enumerate()
        {
            ...
            super::accelerate::sgemm(m, n, k, 1.0, lhs_batch, rhs_batch, 0.0, o);
        }
