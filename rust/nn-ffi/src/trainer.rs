...
use crate::{GpuTensor, Shape, DType, CudaResult, CudaError, Stream};
...
impl GpuTrainer {
    ...
    pub fn init_optimizer_states(&mut self, param_sizes: &[usize]) -> CudaResult<()> {
        self.m_states.clear();
        self.v_states.clear();
    ...
    pub fn init_rng(&mut self, batch_size: usize, initial_seed: u64) -> CudaResult<()> {
    ...
    pub fn optimizer_step(
        ...
        if param_idx >= self.m_states.len() {
            return Err(CudaError::NOT_AVAILABLE);
        }

        crate::tensor_adamw_step(
            param,
            grad,
    ...
    pub fn decode(
        ...
        match self.config.decoding {
            DecodingStrategy::Greedy => {
                crate::tensor_argmax(logits, output, stream)
            }
            DecodingStrategy::Sample { temperature } => {
                if let Some(ref seeds) = self.rng_seeds {
                    crate::tensor_sample(logits, output, seeds, temperature, stream)
                } else {
            ...
            DecodingStrategy::TopK { k, temperature } => {
                if let Some(ref seeds) = self.rng_seeds {
                    crate::tensor_topk_sample(logits, output, seeds, k, temperature, stream)
                } else {
                    crate::tensor_argmax(logits, output, stream)
                }
            }
            DecodingStrategy::TopP { top_p, temperature } => {
                if let Some(ref seeds) = self.rng_seeds {
                    crate::tensor_topp_sample(logits, output, seeds, top_p, temperature, stream)
                } else {
                    crate::tensor_argmax(logits, output, stream)
                }
            }
    ...
    pub fn get_loss(&self, loss_tensor: &GpuTensor) -> CudaResult<f32> {
