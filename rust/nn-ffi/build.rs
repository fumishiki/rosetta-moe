...
fn main() {
    // Shared CUDA kernels location (relative to this crate)
    let manifest_dir = PathBuf::from(env::var("CARGO_MANIFEST_DIR").unwrap());
    let cuda_dir = manifest_dir.join("../../cuda").canonicalize()
        .unwrap_or_else(|_| manifest_dir.join("../../cuda"));

    println!("cargo:rerun-if-changed={}", cuda_dir.join("kernels").display());
    println!("cargo:rerun-if-changed={}", cuda_dir.join("stub.c").display());
    ...
    let kernel_dir = cuda_dir.join("kernels");
